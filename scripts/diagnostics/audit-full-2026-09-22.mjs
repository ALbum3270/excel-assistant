...
async function check(id, fn) {
  try {
    results.push({ id, fixed: true, evidence: await fn() });
  } catch (error) {
    results.push({ id, fixed: false, error: error.stack });
    process.exitCode = 1;
  }
}
...
await check("F03-config-reload-leaves-turn-in-flight", async () => {
  ...
  const daemon = evaluate(
    cut(source, "async function restartSession(", "function handleAgentMessage("),
    {
      ...
      bridge: {
        sendAssistantEvent: (event) =>
          pane.handleServerMessage({ type: "assistant_event", ...event }),
      },
  ...
  return {
    ...
    queued: state.queue.map((t) => t.text),
    dispatched: sent.filter((m) => m.type === "user_message").map((m) => m.text),
    N06FixedInNewPane: true,
  };
});
...
await check("F04-native-path-hash-collides", async () => {
  ...
  return {
    contexts: ids,
    affectedStore: "customWorkbookId uses this URL hash without a document-token suffix",
  };
});
...
await check("F05-history-truncation-splits-one-restore", async () => {
  ...
  const custom = Array.from({ length: 119 }, (_, i) => ({
    id: `recent-${i}`,
    toolCallId: `call-${i}`,
    at: i + 3,
  ...
  const ctx = evaluate(
    source
      .slice(source.indexOf("function compactSnapshotGroup("))
  ...
  return {
    snapshotsStillStored: pi.map((s) => s.id),
    actuallyRestored: restored,
    reportedCommitStatus: result.commitStatus,
  };
});
...
await check("F07-stop-does-not-cancel-manual-restart", async () => {
  ...
  let spawned = 0;
  const ctx = evaluate(
    cut(source, "let restartTimer = null;", "async function findInitialWorkspace") +
      cut(
        source,
...
const report = {
  ...
  note: "Post-fix verification using isolated fixtures; no live Excel or model calls.",
  results,
};
if (process.argv.includes("--save"))
  writeFileSync(
    new URL("../../docs/full-project-audit-2026-09-22.repro.json", import.meta.url),
