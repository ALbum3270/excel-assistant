...
if (sourceStatus) {
  throw new Error(
    "pi-for-excel checkout has uncommitted changes; refusing a non-reproducible build",
  );
}
  ...
  plugins: [
    {
      ...
      setup(builder) {
        ...
        builder.onLoad({ filter: /change-tracker\.ts$/ }, (args) => {
          ...
          return {
            contents: patchedChangeTracker,
            loader: "ts",
            resolveDir: dirname(changeTrackerFile),
          };
        });
  ...
  plugins: [
    {
      ...
      setup(builder) {
        ...
        builder.onLoad({ filter: /format-state\.ts$/ }, (args) => {
          ...
          return {
            contents: patchedFormatState,
            loader: "ts",
            resolveDir: dirname(formatStateFile),
          };
        });
        builder.onResolve({ filter: /\/host\/current\.js$/ }, () => ({
          path: "host-current",
          namespace: "pi-stub",
        }));
        builder.onResolve({ filter: /\/storage\/local\/app-storage\.js$/ }, () => ({
          path: "app-storage",
          namespace: "pi-stub",
        }));
...
// The daemon uses the same upstream queue, without importing browser tools.
await build({
  absWorkingDir: sourceRoot,
  stdin: {
    contents: 'export { createWorkbookCoordinator } from "./src/workbook/coordinator.ts";',
    resolveDir: sourceRoot,
    loader: "ts",
  },
  outfile: join(projectRoot, "daemon", "vendor", "pi-coordinator.mjs"),
  bundle: true,
  format: "esm",
  platform: "node",
  target: "node20",
  banner: {
    js: `// Generated by scripts/vendor-pi-context.mjs. Source: tmustier/pi-for-excel @ ${commit}\n// MIT (c) 2026 Thomas Mustier; see pi-coordinator.LICENSE.`,
  },
});
await copyFile(
  join(sourceRoot, "LICENSE"),
  join(projectRoot, "daemon", "vendor", "pi-coordinator.LICENSE"),
);
