...
`evals/run_spreadsheetbench.py` 在真实 Excel 中运行 [SpreadsheetBench](https://github.com/RUCKBReasoning/SpreadsheetBench) Verified-400 题目，用基准自带的比对代码评分。题目提示词使用官方版本，只加一行说明改为操作打开着的工作簿。评测框架固定每次运行的配置，区分基础设施故障和助手本身的失败，并统计答案区域之外的改动。

| 运行                                   | 模型          | 题数 | 通过 |
| -------------------------------------- | ------------- | ---- | ---- |
| 2026-09-19，固定 10 题样本             | qwen3.7-flash | 10   | 4    |
| 2026-09-19，同样 10 题，下一轮改进之后 | qwen3.7-flash | 10   | 5    |

这 10 题中有 7 题两次结果不同，而且两个方向都有：小模型、小样本下，两次运行之间的波动大于差异本身，所以两个数字都说明不了提升。作为参照，官方榜单上最好的开源方案（fabric-rlm + MiniMax M3）在全部 400 题上达到 82.5%，但它是用 Python 直接改 `.xlsx` 文件，而不是操作打开着的 Excel，两者不能直接比较。
