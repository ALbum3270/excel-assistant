...
![Excel 任务窗格中的一次写入：助手把 G2 里的文本字符串改成真正的公式，工具卡显示提交状态、范围、校验方式、工作簿版本号、该单元格的前后值，以及恢复按钮。](docs/images/write-receipt.png)

*每次改动都会给出回执：提交状态、范围（点一下在 Excel 里选中它）、校验方式、工作簿版本号、变动单元格的前值与后值，以及写入前自动创建的恢复点。*
...
- **撤销多种修改**：恢复点覆盖值、公式、格式、排序、清空、行列、工作表、表格、工作表筛选、隐藏行列、冻结窗格、单元格批注，以及图表创建和部分属性更新。恢复之后还能重做。
...
- **过程可追溯**：每个工作簿有独立的对话历史和可移出的只读档案、恢复点列表、指令队列，界面上还能看到模型提供方、token 用量和估算费用。
...
不做任何配置时，默认使用你登录的 Claude Code。可以在任务窗格的 **Setup → Model connection** 填入兼容接口地址、密钥和三档模型 ID；保存后本地 daemon 自动重启。也可以把 `.env.example` 复制为 `.env` 再填写。这个文件只对本项目生效，不会影响你全局的 Claude Code。以 Qwen 为例：
...
- 每次写入都会显示一张卡片：改了哪个区域、是否已提交、如何验证、有几个公式错误，以及恢复入口。小范围单元格修改还会显示前后值。
- 助手还在执行时，可以先把下一条指令排进队列。
- **History**（聊天页顶部按钮）：列出这个工作簿的历史对话，可以重新打开、继续、导出或删除。导入的对话档案仅供查看，不能由智能体原样续接。
- **Backups** 页：恢复点列表，可搜索、恢复、删除和清空。也可以直接对它说“撤销你刚才的修改”。

![Backups 页列出的恢复点，每条带操作类型、覆盖的范围、单元格数量和恢复按钮。](docs/images/backups.png)
- **Setup** 页：选择工作区文件夹和参考文件，设置模型提供方，查看上一轮和当前智能体运行的 token 用量及估算费用，并可打开 **Ask before changing the workbook** 来逐次审批修改。
