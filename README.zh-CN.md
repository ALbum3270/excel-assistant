...
| 运行                                                  | 模型          | 题数 | 通过 |
| ----------------------------------------------------- | ------------- | ---: | ---: |
| 2026-09-24，分层抽样（seed 20260924），最近一轮修复前 | qwen3.7-flash |  100 |   52 |
| 2026-09-25，修复后**仅重跑上轮未通过的 48 题**        | qwen3.7-flash |   48 |   13 |

第二轮包含中断后补完的 3 题；其余结果为答案不符 28 题、执行或判分异常 5 题、助手未完成 2 题。首轮通过的 52 题**没有在修复后重跑**。52＋13 只说明两个代码版本合计有 65 道不同题目曾至少通过一次，**不是当前版本实测的 65% 通过率**。目前既没有完成当前版本的整套 100 题，也没有完成全部 400 题；这些本地结果不是官方榜单成绩。基准只判答案区域；新通过的 13 题中有 3 题还检测到区域外改动，需要另行复核。修复内容和结果边界见 [100 题分析后的修复](docs/product-fixes-2026-09-25.md)。
