...
function cut(text, a, b) {
  const start = text.indexOf(a), end = text.indexOf(b, start + a.length);
...
test('copy recovery uses the matrix size when the source is a single starting cell', async () => {
  ...
  const range = (state) => ({ ...state, isNullObject: false, load() {}, getCell() {
    return { getResizedRange: (r, c) => range({ ...state, rowCount: r + 1, columnCount: c + 1 }) };
  } });
  const functions = run(
    ...
    cut(recovery, 'async function captureRange(', 'async function captureRangeSnapshot('), {
      ...
      notedCells: () => [], MAX_COMMENT_CAPTURES: 20, CELL_FORMAT_PROPERTIES: {},
      resolveWorksheet: async (_ctx, target) => ({ a1: target.address, worksheet: { getRange: (a) => range(ranges[a]) } }),
    });
  const plan = functions.recoveryPlan('excel_set_cell_range', {
    sheetId: 1, range: 'A1', cells: [[{ value: 1 }, { value: 2 }], [{ value: 3 }, { value: 4 }]],
    copyToRange: 'D1', allow_overwrite: true,
  });
...
test('grouped row recovery reverses its execution order on every undo and redo', async () => {
  ...
  const snapshots = [
    { id: 'structure', toolCallId: 'delete-row', workbookId: 'book', snapshotKind: 'modify_structure_state',
      address: 'Sheet1!2:2', changedCount: 1,
      modifyStructureState: { kind: 'rows_present', position: 2, count: 1, data: 'B' } },
    { id: 'values', toolCallId: 'delete-row', workbookId: 'book', snapshotKind: 'range_values',
      address: 'Sheet1!A1:A3', changedCount: 3, beforeValues: [['A'], ['B'], ['C']], beforeFormulas: [['A'], ['B'], ['C']] },
  ];
  const restore = run(cut(pi, 'function resolveSnapshotKind(', '// src/workbook/recovery/log-store.ts'));
  const append = (snapshotKind) => async (args) => {
    const item = { ...args, id: `inverse-${++serial}`, snapshotKind, workbookId: 'book', at: serial };
    snapshots.unshift(item); return item;
  };
  const dependencies = {
    ...
    applySnapshot: async (_address, values) => {
      ...
      values.forEach((row, i) => { cells[i] = row[0]; });
  ...
  const history = run(recovery.slice(recovery.indexOf('export async function workbookHistory(')).replace('export async', 'async'), {
    ...
    recoveryLog: { restore: (id, options) => restore.restoreWorkbookRecoverySnapshot({
      snapshot: snapshots.find((s) => s.id === id), scope, dependencies, ...options,
    }) },
    Date: { now: () => serial + 1000 },
  });
  const clean = () => { const copy = [...cells]; while (copy.at(-1) === '') copy.pop(); return copy; };
  let id = 'structure';
  for (let i = 0; i < 6; i++) {
...
test('cancelling a signal-aware COM info request leaves writes unlocked at the same revision', async () => {
  ...
  const client = {
    listTools: async () => ({ tools: [{ name: 'excel_range', inputSchema: {
      type: 'object', properties: { action: { type: 'string' }, workbook: { type: 'string' } },
    } }] }),
    getInstructions: () => '',
    callTool: (_request, _schema, { signal }) => new Promise((_resolve, reject) => {
  ...
  const next = await execution.run(path, { write: true, expectedRevision: 0 }, async () => ({ success: true }));
