...
- 写入前自动创建恢复点，可用 `excel_workbook_history` 恢复。覆盖值、公式、格式、排序、清空、插删行列、增删和改名工作表，以及表格创建/加行、工作表自动筛选、隐藏行列、冻结窗格和部分图表操作。以下操作仍没有完整恢复点：
  - 复制工作表（撤销方法是删除副本）、图表删除/数据源、透视表、批注；
  - 所有 COM 写入。表格、筛选、隐藏/冻结的新恢复路径尚待真实 Excel 验收。
