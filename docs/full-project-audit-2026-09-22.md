...
本轮没有修改业务代码。只新增了隔离复现脚本和证据 JSON；没有调用模型、没有运行 40 题评测，也没有打开或修改真实 Excel 工作簿。
...
以下 9 项都有可重复的最小证据。P1 应在下一轮完整模型评测前修复；P2 不影响模型实际收到的工作簿内容，可随后处理。
...
- 原 6 项复核：29/29 通过。
- Node 全套：191/191 通过。
- Python 评测单元测试：10/10 通过。
- React 任务窗格生产构建：通过。
- 新问题复现：JavaScript 7/7、Python 2/2。
- 未运行 40 题模型评测；这些问题应先修复，再用评测验证最终正确率。
- Office.js 行为相关的旧 6 项仍以隔离 Office 契约桩为主；本轮新问题中 F01–F09 均可在不连接真实 Excel 的边界复现。F04 的触发条件是宿主返回原生 Windows 路径且文件名包含 `#`。
...
工作区另有一个 0 字节、名称明显来自失败编辑命令的根目录文件：`rites preserve existing values in cells that only change style…`。它不参与运行，但提交前应删除。此前的 `docs/additional-bug-audit-2026-09-22.md` 仍保留“6 项尚未修复”的历史表述，本报告的实测状态优先。
