...
| P0 | **更强的起始上下文。** 自动提供有限的工作簿蓝图、当前选区附近的实际值/公式和最近修改，减少模型先猜后读。 | `src/taskpane/context-injection.ts`、`src/context/blueprint.ts`、`src/context/selection.ts`、`src/context/change-tracker.ts`。 | `daemon/index.mjs` 每轮只有文档和最多 80 字的选区描述；真实单元格内容要模型主动调用工具。注入必须遵守本项目现有的单元格预算。 |
...
先修正 `evals/run_spreadsheetbench.py` 的授权描述：`answer_position` 是答案区域，但题目可以明确要求额外修改（118-50 同时要求排序 A 列）。随后让评测保存首个偏差单元格、预期与实际值/公式、每次写入的提交和验证状态。已有 7 题结果是 2/7 通过，不能把这一小样本当全量成功率；其中一部分失败是任务理解或模型推理，不能再归咎于基础设施。
