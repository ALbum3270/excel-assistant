...
| 部分完成 | **执行层编辑范围约束。** 模型给出的 `allow_overwrite` 不是用户授权证明；破坏性工具也需要统一范围和工作簿身份。 | `src/tools/with-workbook-coordinator.ts` 集中区分读写并串行 mutation；Pi 的范围授权仍主要依赖上层策略，不能照搬为完整解法。 | 覆盖写入、复制、清空、格式、排序和尺寸调整已绑定本轮选区、`answer_position` 或明确目标范围；工作表结构、对象和 COM 写入还需单独的授权模型。 |
