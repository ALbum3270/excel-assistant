...
范围：当前工作区代码、项目配置、现有测试、已有 SpreadsheetBench 产物，以及用户补充的开发对话文件 `claude记录`（960行）。本文是分析和实施候选，不是已经完成的优化。没有启动新的真实 Excel 评测，没有调用模型，没有修改运行代码。

版本说明：前十七节记录优化前基线的问题，代码位置以基线 `be7e92c` 为准。用户随后补充的 Claude 修复对话及 `fix/lifecycle-hangs` 工作区补丁，另见第十八节；不能把已开始修复的问题继续视为完全未处理，也不能把补丁存在视为全部验收通过。本报告所述“没有调用模型、没有修改运行代码”指本次 Codex 分析；Claude 对话中另有真实模型探针记录。
