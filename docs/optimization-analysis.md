...
**四十六、Claude 后续清单的代码补齐与边界**

Claude 的两笔提交先加入了小范围单元格前后对比，以及图表创建/部分更新的恢复点；其未提交的用量界面把 SDK 的累计 `total_cost_usd` 逐轮相加，会重复计费。本次改为读取每次最新累计值，并以 SDK 的 `modelUsage` 展示当前运行中含子代理的累计 Token 和估算费用。SDK 提供上下文窗口大小，但每轮 Token 是多次模型调用的合计，不等于当前上下文占用，因此不显示虚假的占用百分比。溢出时自动尝试一次 `/compact`，再要求代理先检查工作簿、只完成剩余操作，避免直接重放原始写入指令。

恢复点扩展用项目自己的 IndexedDB 状态与 Pi 日志合并：创建表格撤销时转换成普通区域并恢复原值/格式；加行撤销时检查表格行数再删除目标行；工作表 AutoFilter 保存区域及条件；隐藏行列逐行/列保存原状态；冻结窗格保存行列数并一次恢复。图表删除、图表来源、透视表、批注、复制工作表和 COM 写入仍没有完整撤销方案。新路径目前只有代码检查，真实 Excel 恢复/重做尚待验收。

会话配置指纹加入系统提示、所配置技能文件、模型映射、MCP 形态和相关工具实现。History 可导出用户可见的对话事件 JSON；导入后以只读档案查看，**不伪装成 SDK 可续接的原生会话**。设置页可保存兼容接口地址、凭据与三档模型，Electron 托盘自动重启 daemon 应用新配置。当前只管理一套连接；会话导出不含 SDK 内部上下文。十二项逐项状态和未验收项见 `docs/priority-roadmap.md`。

**四十七、审计修复的三处补修（2026-09-22）**

复查 `7627252` 时发现三条遗漏：单起点二维写入后复制的备份尺寸不足，连续撤销/重做的顺序不再反转，以及 COM 前置 info 真正响应取消信号时错误锁定工作簿。详见 `docs/bug-fix-recheck-2026-09-22.md`。

- 复制目标的值/格式捕获传入待写矩阵尺寸，单起点扩展与实际写入保持一致。
- 分组反向快照保存 `restoreOrder`，每次严格反转实际执行顺序；Pi 日志和自定义恢复点都持久化该信息。通过锁定源码的构建补丁生成 vendor，没有直接手改打包产物。
- COM 的前置 info 失败/取消标记为 `not_committed`，零写入不会消耗 revision 或遗留写锁；已经派发的写入保持原有未知结果处理。

三个实际调用链复现先在旧代码上失败，修复后通过；恢复顺序验证连续六次往返、前代快照删除及日志重载。完整 Node 测试 169/169 通过。没有运行真实 Excel 定向验收或重跑 40 题；旧快照已缺失的数据和顺序不会被自动补回。部署本地更新需重启 daemon 并重新加载任务窗格。

**四十八、追加代码排查（2026-09-22，待修复）**

在 `dd6adc7` 上继续排查，新增六个有隔离复现的问题：混合写值/样式时重写原有文本为公式表达式、无表头建表撤销残留数据、工作表名含 `!` 时地址解析错误、CSV 末尾单行空白页丢失、独立回车符拆行，以及 `is_error:true` 的部分结束消息仍续跑队列。详见 `docs/additional-bug-audit-2026-09-22.md`，复现脚本为 `scripts/diagnostics/audit-additional-2026-09-22.mjs`。

六条均在当前源码与隔离边界中复现；公式求值和表格位移依照 Office API 契约，尚待真实 Excel 验收。现有 Node 测试 169/169 通过但没有覆盖这些输入。本轮只添加审计文档、脚本和输出，未修改业务代码，也未重跑 40 题。先处理数据写入和恢复的两条，再处理地址、CSV 和队列判断。

**四十九、审查后续修复（2026-09-23）**

修复上一轮确认的九项问题：正常重启、初始化期间 Stop→Start、恢复点按完整操作组保留、页面与卡片撤销状态同步、恢复分支删除、评测工作表元数据解析、含逗号的表名解析、参考文件部分保存反馈，以及评测空状态计数。Pi 补丁已通过生成器更新 vendor。详见 `docs/bug-fix-followup-2026-09-23.md`。

Node 217/217、Python 15/15、前端生产构建通过；400 条数据集地址字段离线解析通过，两条已复现问题的题目通过参考答案自比较。未运行模型评测或真实 Excel 验收。已被旧代码淘汰的快照无法补回。
