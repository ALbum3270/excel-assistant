...
| P1 | 撤销与恢复点：值、公式、格式、结构、条件格式、批注、图表 | Pi `src/workbook/recovery-log.ts` + `recovery/*`（约 5.8k 行，依赖 typebox 和存储层） | 移植 Pi，自写存储适配层；备选是 MS-Excel-AI-plugin 的快照撤销（约 300 行，只能完整恢复值和公式） |
